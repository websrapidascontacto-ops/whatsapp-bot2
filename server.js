...
// PROXY DE IMÁGENES (Vital para que no salgan expiradas)
      ...
      for (const change of entry.changes || []) {
        ...
        if (value.messages) {
          for (const msg of value.messages) {
            ...
            const messageData = { 
              chatId: senderId, from: senderId, text: msg.text?.body || (msg.type === "image" ? "📷 Imagen" : ""), 
...
app.post("/send-media", upload.single("file"), async (req, res) => {
  try {
    ...
    const messageData = { chatId: to, from: "me", text: "📷 Imagen", mediaUrl: `/uploads/${file.filename}`, source: "whatsapp", timestamp: new Date() };
    await Message.create(messageData);
    broadcastMessage({ type: "sent", data: messageData });
    res.json({ status: "ok" });
