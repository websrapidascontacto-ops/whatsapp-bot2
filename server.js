...
mongoose.connect(process.env.MONGO_URI)
    .then(() => console.log("✅ Mongo conectado - Estabilizado"))
...
async function processSequence(to, node, allNodes) {
    ...
    try {
        if (node.name === "message" || node.name === "ia") {
            ...
            // BUSCAMOS EN media_url (Nombre exacto en tu flow-editor.js)
            const pathMedia = node.data.media_url; 
            if (pathMedia) {
                const domain = process.env.RAILWAY_STATIC_URL || req.get('host');
                // Forzamos URL absoluta para WhatsApp
                ...
                botText = `🖼️ Imagen: ${node.data.caption || "Adjunto"}`;
            } else {
                console.error("❌ El nodo de media no tiene URL");
                return;
            }
        }
...
app.post("/api/upload-node-media", upload.single("file"), (req, res) => {
    ...
    // Retornamos la ruta que WhatsApp usará
...
server.listen(process.env.PORT || 3000, "0.0.0.0", () => console.log("🚀 Server estable"));